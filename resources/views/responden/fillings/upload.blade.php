        ...
        <form action="{{ route('responden.fillings.upload.store', $filling) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            ...
            <div>
                ...
                <textarea name="catatan"
                          ...
                          class="block w-full rounded-xl border border-gray-200 bg-gray-50/50 px-4 py-3 text-sm text-gray-700 placeholder-gray-400 focus:border-sky-300 focus:bg-white focus:ring-2 focus:ring-sky-500/10 transition @error('catatan') border-red-300 bg-red-50/50 @enderror">{{ old('catatan') }}</textarea>
                @error('catatan')
                    <p class="mt-1.5 text-xs text-red-600 font-medium flex items-center gap-1">
                        <i data-lucide="alert-circle" class="w-3 h-3"></i>
                        {{ $message }}
